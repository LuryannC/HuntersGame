...
extern "C"
{
	JNIEXPORT void Java_com_epicgames_unreal_GameActivity_LocationUpdate(JNIEnv* jni, jclass class_, jdouble latitude, jdouble longitude, jdouble elevation, jdouble horizontalAccuracy, jdouble verticalAccuracy, jdouble bearing, jdouble speed, jlong time)
	{
		AsyncTask(ENamedThreads::GameThread, [=]()
		{
			...
			FGeoLocationModule::Get().GetPlatformProvider(World)->LocationUpdateDelegate.Broadcast(latitude, longitude, elevation, horizontalAccuracy, verticalAccuracy, bearing, speed, time);
		});
	}
}
...
extern "C"
{
	JNIEXPORT void Java_com_epicgames_unreal_GameActivity_LocationError(JNIEnv* jni, jclass class_)
	{
		AsyncTask(ENamedThreads::GameThread, [=]()
		{
			UWorld* World = nullptr;
			...
			FGeoLocationModule::Get().GetPlatformProvider(World)->LocationErrorDelegate.Broadcast("Location Error", -1);
		});
	}
}
...
extern "C"
{
	JNIEXPORT void Java_com_epicgames_unreal_GameActivity_HeadingUpdate(JNIEnv* jni, jclass class_, jdouble magneticNorth, jboolean calibrated)
	{
		AsyncTask(ENamedThreads::GameThread, [=]()
		{
			FGeoHeading heading;
			...
			FGeoLocationModule::Get().GetPlatformProvider(World)->HeadingUpdateDelegate.Broadcast(heading);
		});
	}
}
#endif
